...
export const contractToToken = async (
  ...
  contractToken.name = await (tokenContract as any).name();
  contractToken.symbol = await (tokenContract as any).symbol();
  contractToken.balance = await (tokenContract as any).balanceOf(signer.evmAddress);
  contractToken.decimals = await (tokenContract as any).decimals();
...
export const balanceOf = async (
  ...
  return contract ? (contract as any).balanceOf(balanceAddress) : null;
};
