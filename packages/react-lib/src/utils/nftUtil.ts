...
export const getResolveNftPromise = (nft: NFT|null, signer: Signer, ipfsUrlResolver?: ipfsUrlResolverFn): Promise<NFT|null> => {
  ...
  const contract = new Contract(nft.address, contractTypeAbi, signer as any);
  const uriPromise = (contractTypeAbi as any).some((fn:any) => fn.name === 'uri') ? (contract as any).uri(nft.nftId)
    : (contract as any).tokenURI(nft.nftId);
  return uriPromise
